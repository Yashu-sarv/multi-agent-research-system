...
from database import get_cached_research, save_research
...
if st.session_state.running and not st.session_state.done:
    ...
    # ── CHECK CACHE FIRST ──
    with st.spinner("🕵️ Checking database for existing research..."):
        cached_data = get_cached_research(topic_val)
    
    if cached_data:
        st.success("✅ Found in database! Loading saved report...")
        time.sleep(1) # smooth transition
        st.session_state.results = {
            "search": cached_data['search_results'],
            "reader": cached_data['scraped_content'],
            "writer": cached_data['final_report'],
            "critic": "Cached results loaded from database. Critic review was performed previously."
        }
    else:
        results = {}
        # ── Step 1: Search ──
        ...
        # ── SAVE TO DATABASE ──
        with st.spinner("💾 Saving research to database..."):
            save_research(
                topic_val, 
                results["search"], 
                results["reader"], 
